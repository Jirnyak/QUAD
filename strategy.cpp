...
#define SIDE_MENU 125 //(WINDOW_WIDTH-WINDOW_HEIGHT)/2
#define QUAD_PART 100
#define MAXA 10000
#define DECAY 10
...
(breakpoint set --file strategy.cpp --line N)
...
class cell 
{
    ...
    public:
        ...
        { 
            ...
            vector<object*> inside; // All objects inside cell 
        }
        void info()
        {
...
class quad //ALL VECTORS AND BUFFERS MANAGEMENT
{
public:
    ...
    int quad_get(int x, int y)
    {
        return x/QUAD_PART+y/QUAD_PART*(WORLD_WIDTH/QUAD_PART);
    }
    void add_object(object* obj) 
    {
        objects.push_back(obj);
        quads[quad_get(obj->x, obj->y)].push_back(obj);
        this->world[obj->x*WORLD_WIDTH+obj->y].inside.push_back(obj);
    }
    void update() 
    {
        for (it_obj = objects.begin(); it_obj != objects.end(); ++it_obj)
        {
            if ((*it_obj)->dead == 0) 
            {
                ...
                //NEW POINTERS
                int new_quad = quad_get((*it_obj)->x, (*it_obj)->y);
                quads[new_quad].push_back(*it_obj);
                index = (*it_obj)->x*WORLD_WIDTH+(*it_obj)->y;
                world[index].inside.push_back(*it_obj);
            }
            else
            {
                //DELETE ALL POINTERS
...
void read_map(string map, vector<cell>& world)
{
    string line;
    ifstream file(map);
    if (!file.is_open()) {
...
int main(int argc, char **argv) //int argc, char **argv
{
    ...
    quad q; //CREATE EVERYTHING WORLD

    read_map("politik.txt", q.world);

    for (it = q.world.begin(); it != q.world.end(); ++it)
    {
        if (it->R > it->G)
        {
            it->control = krigan;
    ...
    while (gena < 1000)
    {
        ...
        if (q.world[drop].inside.empty() and q.world[drop].terra == 1)
        {
            q.add_object((new object(q.world[drop].get_x(),q.world[drop].get_y(), plant)));
            //q.world[drop].inside.push_back(object(q.world[drop].get_x(),q.world[drop].get_y(), plant));
            gena += 1;
        }
    }
    ...
    {
        drop  = randomer(rng, WORLD_WIDTH*WORLD_WIDTH);
        if (q.world[drop].inside.empty() and q.world[drop].water == 0 and q.world[drop].control == krigan)
        {
            q.add_object((new object(q.world[drop].get_x(),q.world[drop].get_y(), mob)));
            gena += 1;
        }
    }
    ...
    {
        drop  = randomer(rng, WORLD_WIDTH*WORLD_WIDTH);
        if (q.world[drop].inside.empty() and q.world[drop].water == 0 and q.world[drop].control == erafia)
        {
            q.add_object((new object(q.world[drop].get_x(),q.world[drop].get_y(), knight)));
            gena += 1;
        }
    }
    ...
    while (gena < 10000)
    {
        ...
        if (q.world[drop].inside.empty() and q.world[drop].terra == 1)
        {
            q.add_object((new object(q.world[drop].get_x(),q.world[drop].get_y(), tree)));
            gena += 1;
        }
    }
    ...
    //q.world[player_n].inside.push_back(object(q.world[player_n].get_x(),q.world[player_n].get_y(), player));
    q.add_object((new object(q.world[player_n].get_x(),q.world[player_n].get_y(), plant)));
    ...
    while (quit == false) 
    {
        ...
        if (SDL_PollEvent(&event))
        {
            if (event.type == SDL_MOUSEBUTTONDOWN)
            {
                ...
                for (it_inside = q.world[player_n].inside.begin(); it_inside != q.world[player_n].inside.end(); ++it_inside)
                    {
                    ...
                    case SDLK_l:
                            ...
                            for (it_inside = q.world[player_n].inside.begin(); it_inside != q.world[player_n].inside.end(); ++it_inside)
                            {
                                if ((*it_inside)->type == player)
                                {
                                    ...
                                    q.add_object((new projectile((*it_inside)->x, (*it_inside)->y,bullet,beam(WINDOW_WIDTH/2.0, WINDOW_HEIGHT/2.0, light_x/1.0, light_y/1.0, 1), mob)));
                        ...
                        for (it_inside = it->inside.begin(); it_inside != it->inside.end(); ++it_inside)
                        {
                            switch ((*it_inside)->type)
                            {
                                case mob:                        
                                    ...
                                    {
                                        //objects_buff.push_back(projectile(it_inside->x, it_inside->y,bullet,beam(it_inside->x, it_inside->y, q.world[it_inside->aim].get_x(), q.world[it_inside->aim].get_y(), 1), knight));
                                        q.add_object((new projectile((*it_inside)->x, (*it_inside)->y,bullet,beam((*it_inside)->x, (*it_inside)->y, q.world[(*it_inside)->aim].get_x(), q.world[(*it_inside)->aim].get_y(), 1), knight)));
                                        (*it_inside)->aim = -1;
                                    }
                                    if ((*it_inside)->aim == -1)
                                    {
                                ...
                                case knight:
                                    ...
                                    {
                                        //objects_buff.push_back(projectile(it_inside->x, it_inside->y,bullet,beam(it_inside->x, it_inside->y, q.world[it_inside->aim].get_x(), q.world[it_inside->aim].get_y(), 1), knight));
                                        q.add_object((new projectile((*it_inside)->x, (*it_inside)->y,bullet,beam((*it_inside)->x, (*it_inside)->y, q.world[(*it_inside)->aim].get_x(), q.world[(*it_inside)->aim].get_y(), 1), mob)));
                                        (*it_inside)->aim = -1;
                                    }
                                    if ((*it_inside)->aim == -1)
                                    {
        ...
        for (int i = 0; i <= tiles_in_window+1; i++)
        {
            for (int a = 0; a < use; a++)   
            {
                pos = pos->side_spiral(k%4);
        ...
        if (map_mod == true)
        {
            while (map_mod == true)
            {
                ...
                text = "x: " + to_string(q.world[player_n].get_x()) + " y: " + to_string(q.world[player_n].get_y());
                ...
                minimap_dot.x = q.world[player_n].get_x()/2-5;
                minimap_dot.y = q.world[player_n].get_y()/2-5;
